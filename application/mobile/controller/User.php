    ...
    */
    public function _initialize()
    {
        ...
        $nologin = array(
            ...
            'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code', 'express','recommend_mobile',
        );
...
//            $this->ajaxReturn(['status'=>1,'msg'=>'您已注册']);
        ...
        if (IS_POST) {
            ...
            if(empty($recommend_mobile)){
                $this->ajaxReturn(array('msg'=>'请填写推荐人手机号'));
            }
...
//        $this->success($data['msg']);
    ...
    public function userinfo()
    {
        ...
        if (IS_POST) {
            ...
            I('post.bank_num') ? $post['bank_num'] = I('post.bank_num') : false; //手机
            I('post.user_bank_name') ? $post['user_bank_name'] = I('post.user_bank_name') : false;
            I('post.user_bank') ? $post['user_bank'] = I('post.user_bank') : false;
            I('post.bank_sheng') ? $post['bank_sheng'] = I('post.bank_sheng') : false;
            I('post.bank_shi') ? $post['bank_shi'] = I('post.bank_shi') : false;
    ...
    */
    ...
    public function points()
    {
        ...
        $condition = array(
            'user_id'=>$this->user_id,
            //'change_time'=>array(array('gt',(time()-7*24*3600)),array('lt',time()),'and'),

        );
        $account_log = M('account_log')->where($condition)->order('change_time desc')->limit("{$Page->firstRow},{$Page->listRows}")->select();
   ...
   /* public function recharge()
    ...
    public function withdrawals()
    {
        ...
        if (IS_POST) {
            ...
            $allow_date = array(1);
            $now_date = date('w');
            if(!in_array($now_date,$allow_date)){

                $this->error('请在周一提交提现申请');
                eixt();
            }
            $data = I('post.');
            $data['user_id'] = $this->user_id;
        ...
        $this->assign('user_info',$this->user);//用户的基本信息
        return $this->fetch();
    }
    ...
    public function recommend_mobile(){

            $recommend_mobile = I('post.recommend_mobile');
            $my_mobile = I('post.my_mobile');
            if($recommend_mobile == $my_mobile){

                return json_encode(array('status'=>-1,'msg'=>'请勿自己推荐自己'));
                exit();
            }

            $res = M('users')->where('mobile',$recommend_mobile)->find();
            if(empty($res)){

                return json_encode(array('status'=>-1,'msg'=>'此用户没有注册'));
                exit();
            }
    ...
    public function user_add()
    {
        if(IS_POST){
            $data = I('post.');
            $res = M('users')->where('mobile='."'$data[recommend_mobile]'".'&& is_lock=0')->find();
            if(!$res){

                $this->error('推荐人不存在');exit();
            }
            $rew = M('users')->where('mobile='."'$data[mobile]'")->find();

            if($rew){

                $this->error('用户已存在');
            }
            $data['recommend_mobile'] = $res['mobile'];
            $data['recommend_id'] = $res['user_id'];
            $data['password'] = encrypt($data['password']);
            $data['paypwd'] = encrypt($data['paypwd']);
            $data['mobile_validated'] = 1;
            $data['reg_time'] = time();
            $data['fenhong_points_num'] = I('fenhong_points_nums');
            unset($data['fenhong_points_num']);
            $data['belong_service'] = $this->user_id;
            $add_res = M('users')->add($data);
            //dump($add_res);die;
            if($add_res && $add_res>0){
                if (I('is_active') == 1) {
                    $rd = $this->active_reg_user(intval($add_res),$this->user_id);
                    if (I('fenhong_points_num') > 0 && $rd) {
                        $count = intval(I('fenhong_points_num'));
                        $this->add_bodan_reg_user(intval($add_res),$count,$this->user_id);
                    }
                }
                //入单
    ...
    private function active_reg_user($id,$reg_user_id){
        ...
        $a =accountLog($reg_user_id, 0,0,$dsc,0,0 ,'',0,0,0, -1500);
        if ($a) {
    ...
    private function add_bodan_reg_user($id,$account,$reg_user_id){
        ...
        $b = accountLog($id, $account*1500,0, "用户报单，增加购物劵");
        if (!$b) {
            return false;
        }
        if ($a) {
    ...
    public function return_other()
    {
        if(IS_POST){
            $data = I('post.');
            $info = M('users')->where('mobile='."'$data[my_mobile]'")->find();
            if(!$info){

                $this->error('转账人信息不存在');
                exit();
            }
            if($this->user['baodanbi']<$data['money']){

                $this->error('您的报单币不足');exit();
            }

            if($this->user['mobile'] == $data['my_mobile']){
                $this->error('请不要自己转给自己');
                exit();
            }
             Db::startTrans();

                $account_array = array('jihuobi'=>['exp','jihuobi+'.-$data['money']]);
               $res1 =  Db::table("tp_users")->where('user_id='.$this->user_id)->update($account_array);
                $account_log1 = array(
                    'user_id'=>$this->user_id,
                    'jihuobi'=> -$data['money'],
                    'change_time' =>time(),
                    'user_mobile'=>$this->user['mobile'],
                    'desc'=>'给会员'.substr_replace($data['my_mobile'],'****',3,4).'转激活券',
                );
                Db::name('account_log')->add($account_log1);
                $account_array2 = array('jihuobi'=>['exp','jihuobi+'.$data['money']]);

               $res2 = Db::table("tp_users")->where('user_id='.$info['user_id'])->update($account_array2);
                $account_log2 = array(
                    'user_id'=> $info['user_id'],
                    'jihuobi'=> $data['money'],
                    'change_time' => time(),
                    'user_mobile' => $this->user['mobile'],
                    'desc' => '收到会员'.substr_replace($this->user['mobile'],'****',3,4).'转激活卷',
                );
                 Db::name('account_log')->add($account_log2);
                if($res1 && $res2){
                    Db::commit();
                    $this->success('恭喜转账成功');exit();
                }else{
    ...
    {
        $mobile = I('post.my_mobile');
       $return_name = M('users')->where('mobile='."'$mobile'")->field('nickname')->find();
        if(!$return_name){

            echo json_encode(array('msg'=>'用户名不存在'));exit();
        }else{

            echo json_encode(array('msg'=>$return_name[nickname]));
        }
    ...
    public function fenhong_points()
    {
        ...
        foreach ($user_list as $key => $value) {
            ...
            $end_fenhong = M('users_fenhong')->where($a_condition)->count();
            $user_list[$key]['active_c_points'] = $end_fenhong;
