...
//			}
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$recommend_mobile,$push_id=0){
    	$is_validated = 0 ;
        ...
        //$map['token'] = md5(time().mt_rand(1,99999));
        $res = M('users')->where(array('mobile'=>$recommend_mobile))->find();
        if(!$res){

            return array('status'=>-1,'msg'=>'您的推荐人手机号无效');
        }else{
            if ($res['is_vip'] != 1) {
               return array('status'=>-1,'msg'=>'您填写的推荐人无推荐您的权利');
            }
            $map['recommend_mobile'] = $recommend_mobile;
            $map['recommend_id'] = $res['user_id'];
        }
    ...
    public function get_account_log($user_id,$account_type = 0){
        ...
        if($account_type == 1){
            $account_log_where['user_money|pay_points|group_money|jihuobi|baodanbi'] = ['gt',0];
        }
        if($account_type == 2){
            $account_log_where['user_money|pay_points|group_money|jihuobi|baodanbi'] = ['lt',0];
        }
    ...
    public function get_fenhong_points($user_id,$account_type = 0){
...
//           if($result['status']==1)
    ...
    public function addUser($user)
    {
        ...
        $recommend_ifo = M('users')->where(array('mobile'=>$user['recommend_mobile']))->find();
        if(!$recommend_ifo){
            return array('status'=>-1,'msg'=>'推荐人无效');
        }
        if($user['mobile'] == $user['recommend_mobile']){
            return array('status'=>-1,'msg'=>'不允许自己推荐自己');
        }
        $user['reg_time'] = time();
        ...
        if(!$user_id){
            ...
            if($pay_points > 0)
                accountLog($user_id, 0 , $pay_points , '会员注册赠送积分'); // 记录日志流水
            return array('status'=>1,'msg'=>'添加成功','id'=>$user_id);
        }
    }
}