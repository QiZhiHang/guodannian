...
class Cart extends Base {
    ...
    public function cart3(){
        ...
        foreach ($order_goods as $key => $value) {
            if (substr($value['goods_sn'], 0,4) == 'JNH0') {
                $flag_teshu = 1;
            }else{
                $flag_normal = 1;
            }
        ...
        if ($flag_teshu == 1 && $flag_normal ==0) {
            //购买特殊产品
            $is_teshu = 1;
        }elseif($flag_teshu == 0 && $flag_normal == 1){
            //购买普通产品
            $is_teshu = 0;
        }else{
            exit(json_encode(array('status'=>-6,'msg'=>'不允许混合购买!','result'=>null)));
        }
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            if ($pay_points || $user_money) {
               ...
               /* if (empty($this->user['user_bank_name']) || empty($this->user['user_bank']) || empty($this->user['user_card_id'])) {
                    exit(json_encode(array('status'=>-8,'msg'=>'请先完善个人信息','result'=>null)));
                }*/
            ...
            if ($is_teshu == 1) {
                if ($this->user['is_vip'] == 0) {
                    if ($car_price['goodsFee'] != 1500) {
                        exit(json_encode(array('status'=>-7,'msg'=>'非VIP会员只能购买一件商品','result'=>null)));
                    }
                }
                /*$fenhong_points = M('users_fenhong')->where(array('fenhong_user_id'=>$this->user_id,'fenhong_end_state'=>0,'fenhong_state'=>2))->count();
                if ($fenhong_points >= 0) {
                    exit(json_encode(array('status'=>-7,'msg'=>'您已经复投超过十单','result'=>null)));
                }
                if ((floor($car_price['goodsFee']/1500) +$fenhong_points)  > 10) {
                    exit(json_encode(array('status'=>-7,'msg'=>'您已经复投单数过多，每人最多可以复投十单','result'=>null)));
                }*/
            }

            if ($car_price['payables'] != 0) {
                exit(json_encode(array('status'=>-7,'msg'=>'请使余额和积分进行支付','result'=>null)));
            }
            if(empty($coupon_id) && !empty($couponCode))
               $coupon_id = M('CouponList')->where("code", $couponCode)->getField('id');
            $orderLogic = new OrderLogic();
            $result = $orderLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$user_note,$pay_name,$is_teshu); // 添加订单
            exit(json_encode($result));            
        }
