...
//            $cartLogic->login_cart_handle($this->session_id,$data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    public function active_fenhong(){
        header("Content-type:text/html;charset=utf-8");
        $id = I('get.id/d');
        $now_week_begin =mktime(0,0,0,date("m"),date("d")-date("w")+1,date("Y"));
        $now_week_end = mktime(0,0,0,date("m"),date("d")-date("w")+1,date("Y"))+7*24*3600;
        ...
        if ($users_fenhong['fenhong_active_state'] == 0) {
            ...
            if (empty($active_fenhong_info)) {
                $this->error("没找到可以激活其他点位的复投点");
            }
            $rs = M('users_fenhong')->where(array('fenhong_id'=>intval($active_fenhong_info['fenhong_id'])))->save(array('fenhong_all_state'=>10));
            if ($rs) {
                $rd = M("users_fenhong")->where(array('fenhong_id'=>$id,'fenhong_user_id'=> $this->user_id))->save(array('fenhong_active_state'=>1));
                if ($rd) {
                    $data = array();
                    ...
                    $this->success("激活成功",U('Home/User/fenhong_points'));
                }else{
                    $this->error("激活失败"); 
                }
            }else{
    ...
    */
    ...
    public function withdrawals(){
    	...
    	if(IS_POST)
    	{
    		...
    		$data = I('post.');
            $array = array(3);
            $withdrawals_week = array(1,2,3,4);
            $now_week = date('w');
            if(!in_array($now_week,$withdrawals_week)){

                $this->error('请选择周一到周四进行提现');
                exit();
            }
            ...
            if($data['withdrawals_type'] == 1){
                ...
                if(M('withdrawals')->add($data)){
                    ...
                    accountLog($this->user_id,0,-$data['money']*0.95,'注册积分提现');
   ...
   public function return_other(){
        if(IS_POST){
            ...
            if (!empty($re_users) && intval($re_users['user_id']) > 0 ) {
                ...
                $rd = accountLog($user['user_id'], -$account, 0, "资金转让给会员：". substr_replace($re_users['mobile'],'****',3,4) .",资金扣除");
                if ($rd) {
                    $rd2 = accountLog($re_users['user_id'], $account, 0, "会员：".substr_replace($user['mobile'],'****',3,4)." 转入,资金增加");
                    if ($rd2) {
                        $this->success("资金转让成功！");
                    }else{
...
/**/
    ...
    //每周返
    public function get_fenhong_result(){
        $config = array(
            0=>200,
            1=>100,
            2=>100,
    ...
    //激活每周返
    public function get_active_fenhong_result(){
        $config = array(
            0=>100,
            1=>100,
            2=>100,
    ...
    public function onekey_fuxiao(){
        $this->success("操作成功");
    }
    public function onekey_active(){
        $now_week_begin =mktime(0,0,0,date("m"),date("d")-date("w")+1,date("Y"));
        $now_week_end = mktime(0,0,0,date("m"),date("d")-date("w")+1,date("Y"))+7*24*3600;
        $i = 0;
        $condition = array();
    ...
    public function getTree(){
        ...
        if ($id >= 0) {
            $user_info  = M('users')->field('user_id,mobile,user_bank_name,fenhong_points_num')->where(array('recommend_id' => $id))->select();
            foreach ($user_info as $key => $value) {
                ...
                $return[] = array(
                    'id'=>$value['user_id'],
                    'name'=> $value['mobile'].''.$value['nickname'].'-是否为VIP: '.'是'.'-是否为代理中心:否-分红点位:'.$value['fenhong_points_num'],
                    'isParent'=>$isParent,
                    );
            }
    ...
    public function active_user(){
        ...
        if ($user['jihuobi'] < 1500) {
            $this->error("您的激活币不足");
        }
        $dsc = '激活会员：'.$user['mobile'].',扣除相应金额!';
        $a =accountLog($this->user_id, 0,0,$dsc,0,0 ,'',0,0,0, -1500);
    ...
    private function add_fenhong_points($user_info){
        ...
        if ($rd) {
            M('users')->where(array('user_id'=>intval($user_info['user_id'])))->setInc('fenhong_points_num',1);
            return true;
        }else{
            return false;
        }
    ...
    public function insert_baodan(){
         if(IS_POST){
            ...
            $b= accountLog($this->user_id,$account*1500,0,"会员通过报单中心入单");
            if ($a) {
                $i = $account;
                while (1) {
                    if ($i <= 0) {
    ...
    public function user_add()
    {
        ...
        if(IS_POST){
            $data = I('post.');
            if($data['recommend_mobile'] == $data['mobile']){

                $this->error('请不要自己推荐自己');exit();
            }
            $info = M('users')->where('mobile='.$data['mobile'].'&&is_lock=0')->find();
            if($info){

                $this->error('你所注册的手机号已经存在');exit();
            }
            $recommend_mobile = $data['recommend_mobile'];
            $recommend_info = M('users')->where('recommend_mobile='.$recommend_mobile.' && is_lock=0')->find();
            if(!$recommend_info){
            ...
            $data['belong_service'] = $this->user_id;
            //$data['mobile_valiated'] = 1;
            $user = new \app\admin\logic\UsersLogic();
...
/**/
    ...
    public function insert_baodan(){
         if(IS_POST){
            ...
            $desc = "给会员：".$t_user['mobile']."入".$account."单";
            $a = accountLog($this->user_id, 0,0, $desc ,0,0 ,'',0,0,-$account*1500,0);
            $b= accountLog($this->user_id,$account*1500,0,"会员通过报单中心入单,增加购物币");
            if ($a) {
                $i = $account;
                while (1) {
                    if ($i <= 0) {
