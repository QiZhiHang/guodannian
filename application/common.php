 ...
 * @return  bool
 */
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = '',$group_money = 0,$type = 0,$baodanbi = 0, $jihuobi = 0){
    /* 插入帐户变动记录 */
    $user_info = Db::name('users')->where('user_id',$user_id)->field('mobile')->find();
    $account_log = array(
        'user_id'       => $user_id,
        ...
        'order_id' => $order_id,
        'order_sn' => $order_sn,
        'user_mobile' => empty($user_info['mobile'])?'':$user_info['mobile'],
    );
    /* 更新用户信息 */
    ...
    $update_data = array(
        ...
        'group_money'       => ['exp','group_money+'.$group_money],
        'baodanbi'       => ['exp','baodanbi+'.$baodanbi],
        'jihuobi'       => ['exp','jihuobi+'.$jihuobi],
    );
    if(($user_money+$pay_points+$distribut_money+$group_money+$baodanbi+$jihuobi) == 0)
        return false;
...
//报单中心，每单返利3%
/*function return_service_money($user_info,$order){
    if (empty($user_info) || empty($user_info['recommend_id'])) {
        return false;
    }
    $re_id = intval($user_info['recommend_id']);
    while (true) {
        $info = M('users')->where(array('user_id'=>$re_id))->field('user_id,is_service,recommend_id')->find();
        if (empty($info)) {
            break;
        }
        if (intval($info['is_service']) == 1) {
             accountLog($info['user_id'], 0, 0, "订单：".$order['order_sn']."获得团队奖励",0,0,'',45,2);
            break;
        }else{
            $re_id = intval($info['recommend_id']);
        }
    }
}*/
//第一次分红加入，确认收货后给推荐人添加分红点
/*function add_first_fenhong_points($user_id,$order_id){
    $user_info = M('users')->where(array('user_id'=>intval($user_id)))->find();
    if (empty($user_info)) {
        return false;
    }
    M('users')->where(array('user_id'=>$user_id))->setInc('is_vip',1);
    if (intval($user_info['recommend_id']) == 0) {
        return false;
    }
    //本周日结束的时间
    $week_last_time = mktime(23,59,59,date('m'),date('d')-date('w')+7-7,date('Y'))+7*24*3600;
    $data = array();
    $data['fenhong_user_id'] = intval($user_info['recommend_id']);
    $data['fenhong_user_mobile'] = $user_info['recommend_mobile'];
    $data['fenhong_addtime'] = time();
    $data['fenhong_from'] = intval($order_id);
    ...
    $data['fenhong_all_state'] = 0;
    $rd = M('users_fenhong')->add($data);
    if ($rd) {
        M('users')->where(array('user_id'=>intval($user_info['recommend_id'])))->setInc('fenhong_points_num',1);
        return true;
    }else{
        return false;
    }
}
function add_second_fenhong_points($user_id,$order_id){
    $user_info = M('users')->where(array('user_id'=>intval($user_id)))->find();
    if (empty($user_info)) {
        return false;
    }
...
function every_return_money(){
    ...
    $condition['fenhong_starttime'] = array('lt',time());
    $condition['fenhong_times'] = array('lt',10);
    $condition['fenhong_nowtime'] = array('lt',$ben_week_start);
    // $condition['fenhong_endtime'] = array('gt',time());
    $fenhong_info = M('users_fenhong')->where($condition)->find();
    if ($fenhong_info) {
        $rd = accountLog($fenhong_info['fenhong_user_id'], 0, $config[$fenhong_info['fenhong_times']], $fenhong_info['fenhong_id']."的第".($fenhong_info['fenhong_times'] + 1).'周分红，获得积分',0,1);
        if ($rd) {
            $data= array();
...
/*function every_return_active_money(){
    $config = array(
        0=>200,
        1=>100,
        2=>100,
    ...
    $benzhou_week_satrt = mktime(23,59,59,date('m'),date('d')-date('w')+7-7,date('Y'));
    $condition = array();
    ...
    // $condition['fenhong_active_endtime'] = array('gt',time());
    $condition['fenhong_active_nowtime'] = array('lt',$benzhou_week_satrt);

    $fenhong_info = M('users_fenhong')->where($condition)->find();
    if ($fenhong_info) {
        $rd = accountLog($fenhong_info['fenhong_user_id'], 0, $config[$fenhong_info['fenhong_active_times']], $fenhong_info['fenhong_id']."的激活状态第".($fenhong_info['fenhong_active_times'] + 1+7).'周分红，获得积分',0,1);
        if ($rd) {
            $data['fenhong_active_nowtime'] = time();
            $data['fenhong_active_times'] = array('exp','fenhong_active_times   +1');
            if ($fenhong_info['fenhong_active_times'] == 9 && $fenhong_info['fenhong_end_state'] == 0) {
...
//对某订单进行激活
function active_fenhong_points($fenhong_id){
    $id = intval($fenhong_id);
    if ($id <= 0) {
        return false;
    }
    $fenhong_info = M('users_fenhong')->where(array('fenhong_id'=>$id))->find();
    if ($fenhong_info) {
        if ($fenhong_info['fenhong_times'] == 7 && $fenhong_info['fenhong_active_state'] == 0) {
            //可以激活
            //本周日结束的时间
            $week_last_time = mktime(23,59,59,date('m'),date('d')-date('w')+7-7,date('Y'))+7*24*3600;
            $data = array();
            $data['fenhong_active_state'] = 1;
            $data['fenhong_active_starttime'] = $week_last_time;
            $data['fenhong_active_endtime'] = $week_last_time+7*24*3600*6;
            $res = M('users_fenhong')->where(array('fenhong_id'=>$id))->save($data);
            if ($res) {
                return true;
            }else{
                return false;
